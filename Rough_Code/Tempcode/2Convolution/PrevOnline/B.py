...
h2=Signal(n)
for i in range(0,n):
    h2.set_value_at_time(i,(1/temp)*(n-i))



system1=LTI_System(h1)
system2=LTI_System(h2)

x=Signal(len(price_list))
...
for i in range(n-1,len(price_list)):
    uma.append(y1.signal[y1.INF+i])
    wma.append(y2.signal[y2.INF+i])
