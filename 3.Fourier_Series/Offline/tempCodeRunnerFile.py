...
class FourierSeries:
    def __init__(self, func, L, terms=10):
        ...
        """
        self.func = func  # Target function f(x)
        self.L=L
        self.terms=terms
    ...
    def calculate_a0(self, N=1000):
        ...
        y=self.func(x)
        a0=(1/(2*self.L))*np.trapezoid(y,x)
    ...
    def calculate_an(self, n, N=1000):
        ...
        an=(1/(2*self.L))*np.trapezoid(y,x)
    ...
    def calculate_bn(self, n, N=1000):
        ...
        bn=(1/(2*self.L))*np.trapezoid(y,x)
    ...
    def approximate(self, x):
        ...
        for n in range (1, self.terms()):
            ...
            res+=an*np.cos((n*np.pi*x)/self.L)
            res+=bn*np.sin((n*np.pi*x)/self.L)
        return res
...
def target_function(x, function_type="square"):
    ...
    if function_type == "square":
        ...
        #x ke 0 to 2pi e shift , then wrap in -pi to pi
        return (x + np.pi) % (2 * np.pi) - np.pi
    
    elif function_type == "triangle":

        #undo the sin, but keeps within -pi/2 to +pi/2
        ...
        return np.where(np.abs(x_wrapped) < 0.05, 1.0, 0.0)
