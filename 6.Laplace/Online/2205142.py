...
class SmartIrrigation:
    ...
    def u_error_A(self, u):
        corrupt= 0.08*np.sin(8*self.t) - 0.02*self.t
        return u+corrupt
    ...
    def ISE (self,h):
        h_ss=self.steady_state(h)
        term= (h_ss - h)**2
        return np.trapezoid(term,self.t)
...
for idx, (name, u) in enumerate(inputs.items()):
    ...
    # --- corrupt ---
    uc=system.u_error_A(u)
    
    U_s_vals = np.array([system.laplace_transform(uc, s) for s in s_list])
    H_s_vals = system.H_s(s_list, U_s_vals)
    h_laplacec = system.inverse_laplace(s_list, H_s_vals)
    print(f"\n  ► {name} Corrupted")
    metrics = system.compute_metrics(h_laplacec)
    for k, v in metrics.items():
        print(f"      {k.replace('_',' ').title():<22}: {v}")
    ...
    fig.suptitle(f"Pump Degradation — {name}", fontsize=13, fontweight='bold')
    ...
    # Corruped Laplace subplot
    axes[1].plot(system.t, uc, 'b--', lw=1.8, label="Input u(t)")
    axes[1].plot(system.t, h_laplacec, color='tomato', lw=2.2, label="Output h(t)")
    axes[1].set_title("Corrupted", fontweight='bold')
